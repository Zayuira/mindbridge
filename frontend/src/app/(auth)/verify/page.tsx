...
function VerifyContent() {
    ...
    return (
      <div className="text-center space-y-6">
        ...
        <p className="text-slate-500">
          Таны бүртгэл амжилттай баталгаажлаа. Одоо та нэвтэрч орох боломжтой.
        </p>
        <Button
  ...
  return (
    <div className="space-y-8">
      ...
      {message && (
        <div className="p-3 bg-emerald-50 text-emerald-600 rounded-lg text-sm text-center font-medium">
          {message}
        </div>
      )}
      ...
      <div className="space-y-6">
        ...
        <div
          className={`p-6 rounded-2xl border-2 transition-all ${emailVerified ? 'border-emerald-500 bg-emerald-50/30' : 'border-slate-100 dark:border-slate-800'}`}
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center ${emailVerified ? 'bg-emerald-100 text-emerald-600' : 'bg-blue-50 text-blue-600'}`}
              >
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {emailVerified ? 'Баталгаажсан' : '6 оронтой код оруулна уу'}
                </p>
              </div>
            </div>
            {emailVerified && (
        ...
        <div
          className={`p-6 rounded-2xl border-2 transition-all ${phoneVerified ? 'border-emerald-500 bg-emerald-50/30' : 'border-slate-100 dark:border-slate-800'}`}
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center ${phoneVerified ? 'bg-emerald-100 text-emerald-600' : 'bg-orange-50 text-orange-600'}`}
              >
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {phoneVerified
                    ? 'Баталгаажсан'
                    : 'Гар утсанд ирсэн кодыг оруулна уу'}
                </p>
              </div>
            </div>
            {phoneVerified && (
